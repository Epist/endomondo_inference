...
class EndoModel(object):
    ...
    def __init__(self, is_training, config, dropinManager):
        ...
        lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=0.5, state_is_tuple=True, activation=tf.nn.relu)
        # Other resonable activation functions include: activation=tf.nn.relu, tf.nn.softplus and activation=tf.nn.softmax
        ...
        #create state of lstm
        if config.num_layers==2:
            self._last_state_1_c = tf.placeholder(tf.float32, [batch_size, size])
            self._last_state_1_h = tf.placeholder(tf.float32, [batch_size, size])
            self._last_state_2_c = tf.placeholder(tf.float32, [batch_size, size])
            self._last_state_2_h = tf.placeholder(tf.float32, [batch_size, size])
            self.state = tuple([tf.nn.rnn_cell.LSTMStateTuple(self._last_state_1_c, self._last_state_1_h), tf.nn.rnn_cell.LSTMStateTuple(self._last_state_2_c, self._last_state_2_h)])
        elif config.num_layers==1:
            self._last_state_1_c = tf.placeholder(tf.float32, [batch_size, size])
            self._last_state_1_h = tf.placeholder(tf.float32, [batch_size, size])
            self.state = tuple([tf.nn.rnn_cell.LSTMStateTuple(self._last_state_1_c, self._last_state_1_h)])
        else:
            raise(exception("Cannot handle more than 2 layers at the moment"))
            #self._last_state_1_c = tf.placeholder(tf.float32, [batch_size, size])
            #self._last_state_1_h = tf.placeholder(tf.float32, [batch_size, size])
            #self._last_state_2_c = tf.placeholder(tf.float32, [batch_size, size])
            #self._last_state_2_h = tf.placeholder(tf.float32, [batch_size, size])
        ...
        outputs, state = tf.nn.rnn(cell, inputs, initial_state=self.state)
    ...
    for step, (x, y) in enumerate(dataGen):
        ...
        if m.num_layers==2:
            feed_dictionary = {m.input_data: x, m.targets: y,
                               m._last_state_1_c: state1_c,
                               m._last_state_1_h: state1_h,
                               m._last_state_2_c: state2_c,
                               m._last_state_2_h: state2_h,
                               }
        elif m.num_layers==1:
            feed_dictionary = {m.input_data: x, m.targets: y,
                               m._last_state_1_c: state1_c,
        ...
        if m.is_training:
            if m.num_layers==2:
                ...
                loss, cost, state1_c, state1_h, _ = session.run([m.loss, m.cost,
                                                                                     m.final_state[0].c,
                                                                                     m.final_state[0].h,
                                                                                     eval_op],
                                                                                     feed_dict=feed_dictionary)
...
def main(argv):
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for i in range(config.max_max_epoch):
            ...
            valid_perplexity = run_epoch(session, mvalid, endo_reader, tf.no_op(), 'valid', epochNum, verbose=True)
            print("Epoch: %d Valid %s: %.6f" % (i + 1, lossType, valid_perplexity))

        test_perplexity = run_epoch(session, mtest, endo_reader, tf.no_op(), 'test', epochNum+1)
