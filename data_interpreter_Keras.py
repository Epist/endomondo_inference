...
class dataInterpreter(object):
    ...
    def generator_for_autotrain(self, batch_size, num_steps, trainValidTest, epoch_size = None):
            ...
            while true:
                dataGen=self.endoIteratorSupervised(batch_size, num_steps, trainValidTest)
                if epoch_size!=None
                    for i in epoch_size:
                        x, y = dataGen.next()
                        yield(x,y)
