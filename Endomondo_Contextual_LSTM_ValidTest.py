...
from dataInterpreter_Endomondo_random_access import dataInterpreter, metaDataEndomondo
...
def run_epoch(session, m, data_interp, eval_op, trainValidTest, verbose=False):
...
def main():
    ...
    endo_reader = dataInterpreter(fn=data_path)
    endo_reader.buildDataSchema(endoFeatures, trainValTestSplit)

    dataShape = endo_reader.dataDim
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for i in range(config.max_max_epoch):
            ...
            valid_perplexity = run_epoch(session, mvalid, endo_reader, tf.no_op(), 'valid')
            print("Epoch: %d Valid Perplexity: %.3f" % (i + 1, valid_perplexity))

        test_perplexity = run_epoch(session, mtest, endo_reader, tf.no_op(), 'test')
