...
class keras_endoLSTM(object):
    def __init__():
        ...
        self.endo_reader = dataInterpreter(fn=data_path, scaleVals=True, trimmed_workout_length=trimmed_workout_len)
        self.endo_reader.buildDataSchema(endoFeatures, targetAtt, trainValTestSplit, zMultiple)
        self.input_dim = endo_reader.getInputDim(targetAtt)
        self.target_dim = endo_reader.getTargetDim(targetAtt)
    ...
    def run_model(self, model):
        ...
        for iteration in range(1, self.max_epochs):
            ...
            checkpoint = ModelCheckpoint(self.model_save_fn, verbose=1, save_best_only=False, save_weights_only=False, mode='auto', period=1)
