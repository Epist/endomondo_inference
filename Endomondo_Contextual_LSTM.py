...
from dataInterpreter_Endomondo import dataInterpreter, metaDataEndomondo
...
class EndoModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    self._input_data = tf.placeholder(tf.float32, [batch_size, num_steps, dataDim])
    self._targets = tf.placeholder(tf.float32, [batch_size, num_steps, dataDim])
    ...
    softmax_w = tf.get_variable("softmax_w", [size, dataDim])
    softmax_b = tf.get_variable("softmax_b", [dataDim])
    logits = tf.matmul(output, softmax_w) + softmax_b #Probably need to change this...
    
    #Need a new loss function here...
    loss = tf.nn.weighted_cross_entropy_with_logits(
        [logits],
        tf.reshape(self._targets, [-1, batch_size*num_steps, dataDim]), 
        pos_weight)
...
def run_epoch(session, m, data_interp, eval_op, verbose=False):
  ...
  dataGen=data_interp.endoIterator(m.batch_size,m.num_steps)#A generator over the endomondo data
  ...
  for step, (x, y) in enumerate(dataGen):
    ...
    if verbose and step % (epoch_size // 10) == 10:
      ...
      print("%.3f perplexity: %.3f speed: %.0f dpps" %
...
def main():
  ...
  train_reader = dataInterpreter(fn=data_path)
  train_reader.buildDataSchema(endoFeatures)
  #valid_reader = 
  #test_reader = 
    
  dataShape=train_reader.dataDim
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    for i in range(config.max_max_epoch):
      ...
      train_perplexity = run_epoch(session, m, train_reader, m.train_op,
                                   verbose=True)
